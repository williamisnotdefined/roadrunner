...
`init` creates `.roadrunner/config.json`, `.roadrunner/prompts/`, and `.roadrunner/logs/`. Autonomous `run` builds an in-memory queue from `GOALS.md`, the configured roadmap, and current repository state at every run start.
...
`check` validates `GOALS.md` and the configured provider CLI. `status` derives a roadmap seed queue and shows how many steps are queued, done, blocked, and which step is currently `queue[0]` before provider startup refresh.
...
At run start, `run` cleans registered Roadrunner-owned subprocesses, reads `GOALS.md`, reads the configured roadmap, ignores stale queue files, and asks the provider in read-only planning mode for an in-memory queue proposal from roadmap plus current repository state. For each remaining queued step, `run` performs:
...
`GOALS.md` is loaded once at the start of `run` and remains the in-memory goal for that execution. The configured roadmap is read at run start. The queue is in-memory runtime state and is discarded when the execution ends, so deleting or leaving stale queue files behind does not affect the next `run`. `--goals` and `--roadmap` are resolved relative to the target project directory.
...
Validates that `GOALS.md` exists, is not empty, and the configured provider CLI is available.

### `status`

Prints queued, done, and blocked counts from the roadmap seed queue, followed by the current `queue[0]` step before provider startup refresh.

### `next`

Prints only the current `queue[0]` step from the roadmap seed queue before provider startup refresh.
...
The runner holds `.roadrunner/roadmap.lock` for the duration of the run to avoid concurrent Roadrunner runs in the same target project. Roadrunner does not block because of unrelated user edits, dirty git state, provider git commands, or verification commands that mutate files.

At run start, Roadrunner reads `GOALS.md` once and uses that immutable in-memory snapshot for startup refresh, planning, implementation, fixing, and reconciliation prompts. It also reads the configured roadmap and asks the startup refresh agent for a read-only queue JSON proposal. The startup refresh agent may keep relevant pending work in `queue` and remove obsolete or superseded work from the run queue, but it may not mark work as `history`; Roadrunner only records history after executing verification. Edits to `GOALS.md` during a run only affect future runs.
...
Each run also writes a debug session directory under `.roadrunner/logs/` with `session.log` for human-readable events and `events.ndjson` for machine-readable events. Roadrunner also auto-restarts idle task attempts from planning when no provider activity arrives during planning, implementation, or fixing before the configured idle threshold. Restarting or stopping does not reset or revert project files; it only stops registered Roadrunner subprocesses and, for restarts, repeats the task attempt.
...
Autonomous `run` can also start from a more strategic roadmap that is not in this operational format. In that case startup refresh asks the provider to compile the roadmap and current repository state into a valid in-memory queue proposal before execution.
...
- The configured roadmap is read at run start; the autonomous run queue is in-memory runtime state.
- Startup refresh and reconciliation must not edit files; they return queue JSON proposals.
...
- Concurrent `run` executions are rejected by the configured lock file.
