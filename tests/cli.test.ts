...
describe("cli", () => {
  ...
  test("formats run progress events", () => {
    ...
    const output = [
      ...
      formatRunEvent({ command: ["opencode", "run", "<prompt>"], debug: true, logPath: "/tmp/plan.log", pid: 123, processTreeRoot: null, role: "plan", step, type: "provider-start" }),
      formatRunEvent({ command: ["opencode", "run", "<prompt>"], debug: false, logPath: "/tmp/missing.log", pid: null, processTreeRoot: null, role: "plan", step, type: "provider-start" }),
