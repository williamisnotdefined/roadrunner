...
import { hasLinuxAncestorPid, linuxProcessKey, linuxProcessSnapshotFromStat, readLinuxProcessSnapshot, sameLinuxProcess } from "../src/infrastructure/linux-process.js";
import { processTreeExists, processTreeOwnerEnvKey, processTreeSnapshotKeys, readCurrentProcessTreeRoot, signalProcessTree } from "../src/infrastructure/process-tree.js";
...
describe("Linux process tree helpers", () => {
  ...
  test("parses Linux process metadata from proc stat content", () => {
    const stat = processStat({ name: "node worker (busy)", parentPid: 7, pid: 123, processGroupId: 789, startTimeTicks: "9999" });

    expect(linuxProcessSnapshotFromStat(123, stat)).toEqual({ parentPid: 7, pid: 123, processGroupId: 789, startTimeTicks: "9999" });
  ...
  test("checks Linux process ancestry and identity", () => {
    const snapshots = new Map([
      [1, { parentPid: 999, pid: 1, processGroupId: 1, startTimeTicks: "a" }],
      [2, { parentPid: 1, pid: 2, processGroupId: 1, startTimeTicks: "b" }],
      [3, { parentPid: 2, pid: 3, processGroupId: 1, startTimeTicks: "c" }],
    ]);
...
function processStat({ name = "node", parentPid, pid, processGroupId, startTimeTicks }: { name?: string; parentPid: number | string; pid: number; processGroupId: number | string; startTimeTicks: string }): string {
  return `${pid} (${name}) ${["S", String(parentPid), String(processGroupId), "456", ...Array(15).fill("0"), startTimeTicks].join(" ")}`;
}
