...
describe("restart policy", () => {
  test("resolves config defaults, env overrides, and disabled values", () => {
    expect(resolveAutoRestartPolicy({}, {})).toEqual({ enabled: true, idleMs: 600000, maxRestarts: 10 });
  ...
  test("rejects invalid env overrides and formats block reasons", () => {
    ...
    expect(automaticRestartBlockedReason({ enabled: true, idleMs: 600000, maxRestarts: 10 })).toBe("Provider idle for 10m00s after 10 automatic restarts.");
  });
});
